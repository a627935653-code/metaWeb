...
const BoxPriceChangeRuleAdmin = () => {
  ...
  const getConfigList = async () => {
    const res = await fetchGET({
      path: "/priceConfiguration/box-setting-lst",
    });
  ...
  const handleSave = async (id: number) => {
    ...
    const res = await fetchPost({
      path: "/priceConfiguration/box-setting",
      body: JSON.stringify({
        id: currentRule.id,
        max: currentRule.max,
        min: currentRule.min,
    ...
    },
    {
      ...
      render: (_: any, record: Rule) => (
        <Button
          type="link"
          style={{ padding: 0 }}
          onClick={() => console.log("查看 ID:", record.id, record)}
        >
          查看
        </Button>
      ),
  ...
  return (
    <div>
      <div className="flex justify-between items-center my-1 p-1">
        <div className="text-xl font-bold">盲盒价格变更规则</div>
      </div>
      <Table
        columns={columns}
        dataSource={rules}
...
export default BoxPriceChangeRuleAdmin;
