...
import {
  ...
  activeDateAtom,
  type ViewMode,
} from "@/store/mark-admin";
import useFetch from "@/hooks/useFetch";
import { useKeepAliveECharts } from "@/hooks/useKeepAliveECharts";

// API 返回的数据结构类型
interface ApiChartResponseData {
  listingData: { [key: string]: number | string };
  tradingData: { [key: string]: number | string };
  delistingData: { [key: string]: number | string };
}
...
const TradingVolumeChart = ({ apiUrl }: { apiUrl: string }) => {
  // --- 全局状态管理 ---
  ...
  const transformApiData = (
    apiData: any, // 允许 apiData 为 null 或 undefined
    options: { mode: ViewMode; date: Dayjs }
  ): ChartData => {
    const { mode, date } = options;
    const series: SeriesData = { listings: [], trades: [], delistings: [] };
    let xAxis: string[] = [];
    ...
    const isDataValid =
      apiData &&
      apiData.shelvesData &&
      Object.keys(apiData.shelvesData).length > 0;

    if (isDataValid) {
      // 使用 shelvesData 的 keys 作为 x 轴
      xAxis = Object.keys(apiData.shelvesData);

      xAxis.forEach((key) => {
        // --- 【第2处修改】---
        // 使用正确的字段名来获取数据
        const listingValue =
          parseFloat(String(apiData.shelvesData?.[key])) ?? 0;
        const tradeValue = parseFloat(String(apiData.tradeData?.[key])) ?? 0;
        const delistingValue =
          parseFloat(String(apiData.removedData?.[key])) ?? 0;
  ...
  const chartOptions = useMemo(() => {
    ...
    return {
      ...
      legend: { data: ["上架数量", "成功交易", "下架数量"], top: "10%" },
      grid: {
        left: "3%",
        right: "4%",
        bottom: "10%",
